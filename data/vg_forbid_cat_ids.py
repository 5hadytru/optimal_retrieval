...
# get all unique class names + a map to cat IDs
all_class_names = []

for d in objects_json:
   for o in d["objects"]:
      all_class_names += o["names"]

unique_class_names = sorted(list(set(all_class_names)))

vg_cats = tf.constant(unique_class_names, dtype=tf.string)
forbidden_cats = vg_cats[_is_forbidden_label(vg_cats)]
...
cat_is_forbidden = {
   cat_name: False for cat_name in unique_class_names
}

forbid_count = 0
for cat_name in forbidden_cats:
   cat_is_forbidden[cat_name] = True 
   forbid_count += 1

print(len(unique_class_names), len(forbidden_cats), forbid_count)

with open("vg_forbidden_cat_ids.pkl", "wb") as f:
